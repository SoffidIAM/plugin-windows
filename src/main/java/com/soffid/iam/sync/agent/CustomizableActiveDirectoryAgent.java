							...
							GroupMgr, KerberosAgent
{
	...
	private void performPasswordChange (LDAPEntry ldapUser, String accountName,
			Password password, boolean mustchange, boolean delegation, boolean replacePassword) throws InternalErrorException,
			LDAPException
	{
	...
	private String getDN (ExtensibleObject object)
	{
		...
		{
			return "cn="+object.getAttribute("cn")+","+object.getAttribute("baseDn");
		}
		else
			return dn;
	}
	
	private void createParents (String dn) throws LDAPException, InternalErrorException
	{
		if (dn.equals(baseDN))
			return;
			...
			ExtensibleObject object) throws InternalErrorException
	{
		try {
				...
				if (entry == null) {
					...
					getConnection().add(entry);
					if ((accountName != null) &&
							("user".equals(object.getObjectType()) || 
									"account".equals(object.getObjectType()))) {
					...
					for (String attribute : object.getAttributes()) {
								...
								!"baseDn".equals(attribute))
						{
							...
							if (value == null && entry.getAttribute(attribute) != null) {
								...
								if (value.length != 1 || ! value[0].equals(entry.getAttribute(attribute).getStringValue()))
								{
									modList.add(new LDAPModification(
					...
					if (!entry.getDN().equalsIgnoreCase(dn))
					{
						...
						if ( i > 0)
						{
							...
							getConnection().rename(entry.getDN(), dn.substring(0,i), parentName, true);
						}
					}
				}
		} catch (Exception e) {
	...
	public void removeObjects(ExtensibleObjects objects)
		...
		for (ExtensibleObject object : objects.getObjects()) {
			String dn = getDN(object);
			try {
				if ("user".equals(object.getObjectType()) || "account".equals(object.getObjectType()))
				{
					if (dn != null) {
				...
				if ("role".equals(object.getObjectType()))
				{
			...
			for (ExtensibleObject entry: entries.getObjects())
			{
				try
				{
					...
					if (ldapentry != null)
					{
						conn = new LDAPConnection(new LDAPJSSESecureSocketFactory());
						conn.connect(ldapHost, ldapPort);
						conn.bind(ldapVersion, ldapentry.getDN(), password.getPassword().getBytes("UTF8"));
						conn.disconnect();
						return true;
					}
				}
			...
			throws LDAPException, InternalErrorException
	{
		...
		for (ExtensibleObjectMapping mapping: objectMappings )
		{
			if (mapping.getSoffidObject().equals(type))
			{
				...
				if (any && base != null)
				{
					LDAPSearchConstraints oldConst = getConnection().getSearchConstraints();	// Save search constraints
					LDAPPagedResultsControl pageResult =
							new LDAPPagedResultsControl(getConnection().getSearchConstraints()
								.getMaxResults(), false);
					
					do
					{
						LDAPSearchConstraints constraints = getConnection().getSearchConstraints();
						constraints.setControls(pageResult);
						getConnection().setConstraints(constraints);
					
						LDAPSearchResults search = getConnection().search(base,
								LDAPConnection.SCOPE_SUB, sb.toString(), null, false);
						while (search.hasMore())
						{
							LDAPEntry entry = search.next();
							
							accounts.add(entry.getAttribute("sAMAccountName").getStringValue());
						}
			...
			throws InternalErrorException, LDAPException
	{
		...
		{
			if (objectMapping.getSoffidObject().getValue().equals(account.getObjectType()))
			{
	...
	public Rol getRoleFullInfo(String roleName) throws RemoteException,
		...
		for (ExtensibleObject systemObject: systemObjects.getObjects())
		{
			...
			if (entry != null)
			{
				for (ExtensibleObjectMapping objectMapping: objectMappings)
				{
					if (objectMapping.getSoffidObject().getValue().equals(rolObject.getObjectType()))
					{
	...
	public List<Rol> getAccountRoles(String userAccount)
		...
		try
		{
			...
			userGroups =
					(memberofattr == null ? new String[] {} : memberofattr.getStringValueArray());

			// Process the user groups
			for (int i = 0; i < userGroups.length; i++)
			{
				String group = searchEntry(userGroups[i])
			...
			throws RemoteException, InternalErrorException
	{
		...
		ExtensibleObjects objects = objectTranslator.generateObjects(new UserExtensibleObject(account,
				userData, getServer()));
		...
		try
		{
			for (ExtensibleObject object : objects.getObjects())
			{
				if ("user".equals(object.getObjectType()) ||
						"account".equals(object.getObjectType()))
				{
					updateUserData(userData.getCodi(), object);
	 ...
	 */
	private void updateUserGroups (String userName,
			ExtensibleObject object) throws InternalErrorException,
			LDAPException, UnknownUserException, RemoteException, UnknownRoleException
	{
		...
		String soffidGroups[] =
				((memberofattr == null) ? new String[] {} : memberofattr
						.getStringValueArray());
		HashMap<String, String> h_soffidGroups = new HashMap<String, String>(); // Soffid groups
		for (int i = 0; i < soffidGroups.length; i++)
		{
			LDAPEntry groupEntry = searchEntry(soffidGroups[i]);
			String groupCN = groupEntry.getAttribute(SAM_ACCOUNT_NAME_ATTRIBUTE).getStringValue();
			...
			UnknownRoleException
	{
		...
		synchronized (lc)
		{
			try
			{
				soffidGroup = getServer().getGroupInfo(group, getDispatcher().getCodi());
				eo = objectTranslator.generateObjects(
						new GroupExtensibleObject(soffidGroup, getDispatcher().getCodi(),
								getServer()));
			}
			...
			catch (UnknownGroupException e)
			{
				...
				eo = objectTranslator.generateObjects(
						new RoleExtensibleObject(soffidRole, getServer()));
			}
			...
			lc.modify(groupEntry.getDN(), new LDAPModification(LDAPModification.ADD,
					new LDAPAttribute("member", userEntry.getDN())));
	...
	public void updateUser(String accountName, String description)
		...
		account.setDispatcher(getDispatcher().getCodi());
		ExtensibleObjects objects = objectTranslator.generateObjects(
	...
	public void updateUserPassword(String userName, Usuari userData,
		...
		Account account = new Account();
		account.setName(userName);
		account.setDescription((userData != null) ? userData.getFullName() : userName);
		account.setDisabled(false);
		account.setDispatcher(getDispatcher().getCodi());
		
		ExtensibleObjects objects = null;
		if (userData != null)
			objects = objectTranslator.generateObjects(new UserExtensibleObject(account, userData, getServer()));
			...
			throws InternalErrorException
	{
		try
		{
			...
			object.setAttribute("accountDescription", "Kerberos account for "+server);
				...
				synchronized (lc)
				{
					if (entry == null)
					{
						...
						for (String attribute : ldapObject.getAttributes()) {
							String values[] = toStringArray(ldapObject.getAttribute(attribute));
							if (values != null && !"dn".equals(attribute) &&
									!"baseDn".equals(attribute)  )
							{
						...
						attributeSet.add(new LDAPAttribute("servicePrincipalName", principal));
						log.info("Adding attribute {}={}", "servicePrincipalName", principal);
					...
					lc.modify(entry.getDN(), new LDAPModification[] {
					...
					p.exec("ktpass.exe " + "-princ \"" + result.getPrincipalName() + "\" " + "-out \""
							+ f.getPath() + "\" " + "-pass \"" + result.getPassword().getPassword() + "\" "
							+ "-mapuser \"" + uid + "@" + realm + "\" "
				...
				for (ExtensibleObject object: parsedObjects.getObjects())
				{
					...
					if (user != null)
					{
						...
						if (attributes instanceof Map)
						{
							...
							{
								attributesMap.put((String)attributeName,
										(String) vom.toSingleString(((Map)attributes).get(attributeName)));
							}
